...
- **Email (Resend):** `lib/mailer.ts` sends a notification of each contact/wholesale submission to the OB inbox (`info@obdistributions.com`) via the Resend HTTP API (reliable on serverless/Vercel where SMTP is blocked). Configured via env: `RESEND_API_KEY`, `MAIL_FROM` (must be on the Resend-verified domain), `MAIL_TO`. Best-effort — a mail failure never fails the submission, and with no API key the forms still work (no email sent).
