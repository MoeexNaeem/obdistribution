...
/*
  ScrollExpand (React Bits) — a media frame that opens up to fill the stage as
  you scroll inside/through it. Ported to TypeScript; logic unchanged.
*/
...
const ScrollExpand = ({
  ...
  const propsRef = useRef<Required<
    Pick<
      ScrollExpandProps,
      | "startWidth"
      | "startHeight"
  ...
  // Keep the latest prop values in the ref without mutating during render — the
  // scroll/rAF handlers read propsRef.current at call time.
  useEffect(() => {
    propsRef.current = {
      startWidth,
      startHeight,
